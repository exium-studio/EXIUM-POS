...
import { Award, Gift, Tag, Users, RefreshCw, Plus, Settings, Percent, Calendar, Check, X } from 'lucide-react';
  ...
  const loadData = async () => {
    ...
    try {
      const [mems, proms, config] = await Promise.all([
        ...
        api.get('/loyalty/config').catch(() => null), // fallback if not seeded
      ]);
      setMembers(mems);
      setPromotions(proms);
      if (config) {
